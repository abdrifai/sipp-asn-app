...
import FormInputAgama from "./FormInputAgama";
...
const RefAgama = () => {
  ...
  const getRefAgama = async () => {
    const result = await fetch("/api/referensi/agama");
  ...
  const receviedData = async (data: FormData) => {
    ...
    if (data.id) {
      try {
        const response = await fetch(`/api/referensi/agama`, {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="container pt-8">
      <h1 className="font-bold text-gray-600 text-xl">Referensi Agama</h1>
      {toggel && (
        <FormInputAgama
          onClose={handleToggel}
          onDataReceived={receviedData}
          sendData={agamaSelect}
        />
      )}
      <CardBoxButtonAdd
        title="Daftar Agama"
        ...
        <div className="pt-3">
          <table className="table-auto min-w-full">
            ...
            <tbody className="overflow-y-scroll">
              {agama &&
                agama.map((item: any, index: number) => (
                  <tr
                    key={index}
